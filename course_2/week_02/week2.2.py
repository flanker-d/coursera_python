...
class Polyline:
    ...
    def set_points(self):
        for i in range(len(self._points)):
            self._points[i] += self._speeds[i]
            if self._points[i].x > SCREEN_DIM[0] or self._points[i].x < 0:
                self._speeds[i] = Vec2d(- self._speeds[i].x, self._speeds[i].y)
            if self._points[i].y > SCREEN_DIM[1] or self._points[i].y < 0:
                self._speeds[i] = Vec2d(self._speeds[i].x, -self._speeds[i].y)

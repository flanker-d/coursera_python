...
class Polyline:
    ...
    def get_point(self, points, alpha, deg=None):
        ...
        return Vec2d(Vec2d(points[deg]) * alpha) + self.get_point(points, alpha, deg - 1) * (1 - alpha)
...
class Knot(Polyline):
    ...
    def get_knot(self, points, count):
        ...
        for i in range(-2, len(points) - 2):
            ptn = []
            ptn.append((Vec2d(points[i]) + Vec2d(points[i + 1])) * 0.5)
            ptn.append(points[i + 1])
            ptn.append((Vec2d(points[i + 1]) + Vec2d(points[i + 2])) * 0.5)

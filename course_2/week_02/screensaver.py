...
class Polyline:
    ...
    def set_points(self):
        for i in range(len(self.points)):
            ...
            if (
                self.points[i].x > SCREEN_DIM[0]
                or self.points[i].x < 0
            ):
                self.speeds[i] = Vec2d(
                    -self.speeds[i].x, self.speeds[i].y
                )
            if (
                self.points[i].y > SCREEN_DIM[1]
                or self.points[i].y < 0
            ):
                self.speeds[i] = Vec2d(
                    self.speeds[i].x, -self.speeds[i].y
                )
    ...
    def draw_points(
        self, points, width=3, color=(255, 255, 255)
    ):
        for point in points:
            pygame.draw.circle(
                gameDisplay, color, point.int_pair(), width
            )
...
class Knot(Polyline):
    ...
    def get_points(self, base_points):
        ...
        for i in range(self.count):
            res.append(
                self.get_point(base_points, i * alpha)
            )
        return res
